      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 border-b border-white/10 pb-5 md:flex-row md:items-end md:justify-between">
          <div>
            <Link
              href="/"
              className="text-xs font-black uppercase tracking-[0.24em] text-cyan-300"
            >
              VS ARENA Lobby
            </Link>
        ...
        <section className="grid gap-4 py-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)]">
          <div className="space-y-4">
            <div className="border border-white/10 bg-white/[0.04] p-5">
              <div className="flex items-center justify-between gap-3">
                ...
                <button className="border border-cyan-300 bg-cyan-300 px-4 py-2 text-xs font-black text-black">
                  새 주제 편성
                </button>
              </div>
              ...
              <div className="mt-4 space-y-3">
                {liveItems.map((arena) => {
                  ...
                  return (
                    <div
                      key={arena.id}
                      className="grid gap-3 border border-white/10 bg-black/25 p-4 md:grid-cols-[minmax(0,1fr)_220px]"
                    >
                      <div>
                      ...
                      <div className="grid grid-cols-3 gap-2 text-xs font-black">
                        <button className="border border-amber-300/40 bg-amber-300/10 text-amber-200">
                          메인
                        </button>
                        <button className="border border-white/10 bg-white/[0.04] text-zinc-300">
                          마감
                        </button>
                        <Link
                          href={`/arena/${arena.id}`}
                          className="grid place-items-center border border-cyan-300/40 bg-cyan-300/10 text-cyan-200"
                        >
                          보기
                        </Link>
                      </div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                {queueItems.map((arena) => (
                  ...
                  >
                    ...
                    <div className="mt-2 text-base font-black leading-snug text-white">
                      {arena.title}
                    </div>
          ...
          <aside className="space-y-4">
            <div className="border border-rose-300/30 bg-rose-400/10 p-5">
              <h2 className="text-sm font-black text-rose-100">어뷰징 레이더</h2>
              <div className="mt-4 space-y-2 text-sm font-bold text-zinc-300">
                <div className="flex justify-between border border-white/10 bg-black/25 px-3 py-3">
